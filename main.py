...
def main(
    ...
    sofr, ffr = calib.read_sofr_ffr(ma=True)
    ...
    Theta0 = calib.calibrate(df=sofr, model=model, W=252).iloc[-1].values
    ...
    ThetaT = calib.calibrate(df=sofr, model=model, W=len(sofr)).iloc[-1].values
    Theta0 = dict(zip(calib.PARAM_DICT[model], Theta0))
    ThetaT = dict(zip(calib.PARAM_DICT[model], ThetaT))
    # extract last sofr observation as r0
    r0 = sofr.loc[len(sofr)-1, "r"]

    # simulate [0,T] using params at t=0
    # sample_0 = sim.sim_short_rate(kind=model, a=a0, b=b0, sigma=sigma0, n=n,T=T+Delta, dt=dt, r0=r0)
    sample_0 = sim.sim_short_rate(kind=model, **Theta0, n=n,T=T+Delta, dt=dt, r0=r0)
    ...
    sample_T = sim.sim_short_rate(kind=model, **ThetaT, n=n,T=Delta, dt=dt, r0=ErT)

    # using simulate short rate sample paths, transform to term rates
    L0 = sim.forward_term_rate(
        r=sample_0+ffr[-1], t=0., accr_start=T, accr_length=Delta, dt=dt
    )
    LT = sim.forward_term_rate(
        r=sample_T+ffr[-1], t=(T-T), accr_start=(T-T), accr_length=Delta, dt=dt
    )
    ...
    if loud:
        ...
        for key, val in Theta0.items():
            print(f"\t{key} = {val}")
        print(f"Parameters at t=T:")
        for key, val in ThetaT.items():
            print(f"\t{key} = {val}")
        print("R0:", r0)
        print("E[RT]:", ErT)
...
if __name__ == "__main__":
    ...
    claim = main(
        model="vasicek", N=1,k=0.8,kprime=1.5,n=10000,dt=1/252,
